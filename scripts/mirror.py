#!/usr/bin/env python3
"""Mirror new badness releases from PyPI as commits and tags.

For every badness version on PyPI newer than the one pinned in
...
def current_version() -> str:
    match = re.search(r'"badness==([^"]+)"', PYPROJECT.read_text())
    assert match, "no badness pin in pyproject.toml"
    return match.group(1)


def pypi_versions() -> list[str]:
    with urllib.request.urlopen("https://pypi.org/pypi/badness/json") as response:
...
def set_version(version: str) -> None:
    ...
    pyproject = re.sub(r'"badness==[^"]+"', f'"badness=={version}"', pyproject)
